...
namespace mapa
{
    ...
    public class PathFinder
    {
        ...
        private void InitializeNodes(bool[,] map)
        {
            ...
            Console.WriteLine("Pathfinding initializenodes");
            Console.WriteLine("StartPos is [{0}],[{1}].", this.searchParameters.StartLocation.X, this.searchParameters.StartLocation.Y);
            Console.WriteLine("EndPos is [{0}],[{1}].", this.searchParameters.EndLocation.X, this.searchParameters.EndLocation.Y);
            for (int y = 0; y < this.height; y++)
            {
                for (int x = 0; x < this.width; x++)
                {
                    this.nodes[x, y] = new Node(x, y, map[x, y], this.searchParameters.EndLocation);
                    if(this.nodes[x,y].IsWalkable) Console.Write("0"); else Console.Write("1");
                } Console.WriteLine();
            }
        }
